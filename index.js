...
function getFullNames(runners) {
  /* CODE HERE */
    ...
    runners.forEach(function (item) {
      return name.push(`${item.last_name}, ${item.first_name}`)
    });
    return name
}
...
function firstNamesAllCaps(runners) {
  ...
  runners.map(function (item) {
    return correctName.push(`${item.first_name.toUpperCase()}`)
  });
  return correctName
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
 /* CODE HERE */
  const shirt = runners.filter(function (item) {
...
function tallyUpDonations(runners) {
  ...
  if (runners.length !== 0){
    runners.reduce(function(accumulator, item){
      return donations = accumulator + item.donation
    }, 0)
  } else {return donations = 0}
  return donations
}
